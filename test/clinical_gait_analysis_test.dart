...
void main() {
  group('ClinicalGaitAnalysis', () {
    ...
    test('detects the correct step count per foot', () {
      final report = ClinicalGaitAnalysis.analyze(record);
    ...
    test('reports nonzero stride-time asymmetry when feet differ', () {
      final leftLine = _footLine([100, 0, 100, 0, 100, 0]);
      final rightLine = _footLine([100, 0, 0, 100, 0, 0]);
      final times = [0, 100, 200, 300, 400, 500];
      final asymRecord = InsoleRecord(
        id: 2,
        name: 'asym',
        date: '2026-01-01 00:00',
        time: 1,
        details: [for (final t in times) PressureFrame(time: t, item: emptyGrid())],
        line: leftLine,
        rightLine: rightLine,
        distanceKm: 0,
        pace: 0,
      ...
      final report = ClinicalGaitAnalysis.analyze(asymRecord);
    ...
    test('handles an empty record without throwing', () {
      final empty = InsoleRecord(
        id: 3,
        name: 'empty',
        date: '2026-01-01 00:00',
        time: 0,
      ...
      final report = ClinicalGaitAnalysis.analyze(empty);
